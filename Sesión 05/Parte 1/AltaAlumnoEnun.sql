...
VALUES (seq_matricula.nextval, 'PEPE', 'OFIM', 1);
INSERT INTO matriculas
VALUES (seq_matricula.nextval, 'ANA', 'OFIM', 1);
INSERT INTO matriculas
VALUES (seq_matricula.nextval, 'JUAN', 'OFIM', 1);
INSERT INTO matriculas
VALUES (seq_matricula.nextval, 'LUIS', 'OFIM', 1);

UPDATE grupos
SET plazasLibres=plazasLibres - 4
WHERE asignatura = 'OFIM'
  AND idgrupo = 1;

--Dejamos 1 plaza libre en el grupo 2 de ofimatica
INSERT INTO matriculas
VALUES (seq_matricula.nextval, 'ANTONIO', 'OFIM', 2);
INSERT INTO matriculas
VALUES (seq_matricula.nextval, 'MERCEDES', 'OFIM', 2);
INSERT INTO matriculas
VALUES (seq_matricula.nextval, 'JESUS', 'OFIM', 2);

UPDATE grupos
SET plazasLibres=plazasLibres - 3
WHERE asignatura = 'OFIM'
  AND idgrupo = 2;
...
create or replace procedure matricular(arg_alumno matriculas.alumno%type,
    ...
    GRUPO_LLENO exception;
    PRAGMA EXCEPTION_INIT (GRUPO_LLENO, -20001);

    ASIGNATURA_GRUPO_INEXISTENTE exception;
    PRAGMA EXCEPTION_INIT ( ASIGNATURA_GRUPO_INEXISTENTE, -20002 );

    -- Declaración de nuevos tipos:
    type tGrupo is record
                   (
                       grupo         grupos.idgrupo%type,
                       asignatura    grupos.asignatura%type,
                       plazas_libres grupos.plazaslibres%type
                   );


    -- Declaración de variables:
    grupoSeleccionado tGrupo;
    id_matricula matriculas.idmatricula%type;
    -- Declaración de Cursores
    cursor vGrupo is
        select *
        from grupos
        where asignatura = arg_asig
          and idGrupo = arg_grupo;

begin

    open vGrupo;
    fetch vGrupo into grupoSeleccionado;
    close vGrupo;

    id_matricula := seq_matricula.nextval;

    if grupoSeleccionado.plazas_libres < 1 then
        raise_application_error(-20001, 'No quedan plazas libres en el grupo ' || arg_grupo || ' de la asignatura ' ||
                                        arg_asig || '.');
    end if;

    -- Matriculamos al alumno:
    INSERT INTO matriculas
    VALUES (id_matricula, arg_alumno, arg_asig, arg_grupo);


    -- Actualizamos el número de plazas libres:
    UPDATE grupos
    SET plazasLibres=plazasLibres - 1
    WHERE asignatura = arg_asig
      AND idgrupo = arg_grupo;
...
exception
    when FK_VIOLATED then
        raise_application_error(-20002, 'No existe la combinación ' || arg_asig || '-grupo' || arg_grupo || '.');
        rollback;
...
begin
    --inciacializaciones
    begin
        delete from matriculas where IDMATRICULA is not null;
        delete from grupos where idGrupo is not null;
        delete from asignaturas where NOMBRE is not null;
    ...
    begin
        ...
        FROM grupos
                 left join matriculas on (matriculas.grupo = grupos.idGrupo
