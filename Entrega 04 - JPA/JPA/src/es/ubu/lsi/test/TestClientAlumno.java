...
public class TestClientAlumno {
	...
	static void testService() throws Exception {
		...
		try {
			...
			// ALUMNO: inserta una incidencia con valores erróneos.
			insertarNuevoTipoIncidenciaError(implService);

			// ALUMNO: Inserta una incidencia con la fecha incorrecta.
			insertarIncidenciaConFechaErronea(implService);
	...
	private static void indultarAUnConductorQueNoExiste(Service implService) {
		try {
			...
			try{
				System.out.println("\nALUMNO: Indultar a un conductor con NIF incorrecto");
				implService.indultar("A");
				System.out.println("\tERROR NO detecta que el DNI es inválido y finaliza la transacción");
			}catch (IncidentException ex) {
				if (ex.getError() == IncidentError.STRING_LENGTH_EXCEDED) {
					System.out.println("\tOK detecta correctamente que el DNI no es válido");
				} else {
					System.out.println("\tERROR detecta un error diferente al esperado:  " + ex.getError().toString());
				}
			}
	 ...
	 * @throws Exception error en test
	 */
	private static void insertarNuevoTipoIncidenciaError(Service implService) throws Exception {
		Connection con = null;
		Statement st = null;
		ResultSet rs = null;
		try {
			try {
				System.out.print("Insertar una incidencia demasiado larga\n");
				implService.insertarTipoIncidencia("" +
				...
				if (e.getError() == IncidentError.STRING_LENGTH_EXCEDED) {
					System.out.println("\tOK Comprobación del tamaño del texto de la anotación correcta.");
				} else {
					System.out.println("\tMAL No lanza la comprobación correcta.");
				}
			}

			try {
				System.out.print("Insertar una incidencia con penalización negativa\n");
				implService.insertarTipoIncidencia("" +
						"TEST", -2);

			} catch (IncidentException e) {
				if (e.getError() == IncidentError.ILLEGAL_INT) {
					System.out.println("\tOK Comprobación del tamaño del entero negativo correcto.");
				} else {
					System.out.println("\tMAL No lanza la comprobación correcta.");
				}
			}
	...
	/**
	 * Intenta insertar una incidencia cuyo fecha no es correcta.
	 *
	 * @param implService servicio
	 */
	private static void insertarIncidenciaConFechaErronea(Service implService) {
		try {
			System.out.println("ALUMNO: Insertar incidencia con fecha errónea");
			// fecha y usuario correcto
			implService.insertarIncidencia(dateformat.parse("02/06/2025 17:00"), "10000000A", 2); // 99 NO EXISTE
			System.out.println("\tERROR NO detecta que la fecha es errónea y finaliza la transacción");

		} catch (IncidentException ex) {
			if (ex.getError() == IncidentError.ERROR_IN_DATE) {
				System.out.println("\tOK Detecta correctamente que la fecha es errónea");
			} else {
				System.out.println("\tERROR detecta un error diferente al esperado:  " + ex.getError().toString());
			}
		} catch (PersistenceException ex) {
